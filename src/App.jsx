...
export default function FitnessTracker() {
  ...
  const [profileEdit, setProfileEdit] = useState({ name: "", age: "", weight: "", height: "" });
    ...
    async function load() {
      ...
      try {
        ...
        const { data: pr } = await supabase
          .from("user_data")
          .select("data")
          .eq("user_id", uid + "_profile")
          .maybeSingle();
        if (pr && pr.data) {
          setProfile(pr.data);
          setProfileEdit(pr.data);
        }
      } catch (e) {
  ...
  function WeightLog() {
    ...
    return (
      <div style={S.page}>
        ...
        <div style={{ fontSize: 12, color: MUTED, marginBottom: 20 }}>{sorted.length} записей</div>
        ...
        {sorted.length >= 2 && (
          <div style={S.statGrid}>
            ...
            <div style={S.statCard}>
              ...
              <div style={{ ...S.statNum, color: diff > 0 ? "#c0503a" : diff < 0 ? "#2e8a4a" : ACCENT }}>
        ...
        {sorted.length >= 2 && (
          <div style={{ ...S.card, marginBottom: 16 }}>
            ...
            <MiniChart points={chartPoints} color={ACCENT} unit="кг" />
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
